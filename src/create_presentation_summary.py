...
def build_midterm_presentation_guide(
    ...
    lines = [
        ...
        "- 실행 파일: `02_Run_Admin_Console.bat`",
...
def build_midterm_qna(metrics: dict, threshold_summary: dict) -> str:
    ...
    lines = [
        ...
        "",
        "답변:",
        "이번 발표 목적이 구현 진행 상황과 결과를 보여주는 것이라면 Streamlit Admin 콘솔만으로도 충분합니다. Admin 콘솔 안에 성과 요약, 모델 비교, threshold 조정, SHAP 해석, row 시뮬레이션, CSV MVP, 연구계획이 들어 있기 때문에 발표 흐름 자체가 화면에 준비되어 있습니다. 다만 발표 전에는 `02_Run_Admin_Console.bat`로 미리 실행하고, 백업용으로 주요 PNG와 Markdown 파일 위치를 알고 있으면 안전합니다.",
        "",
        "짧게 말하기:",
...
def build_backup_checklist(
    ...
    lines = [
        ...
        "## 1. 발표 직전 실행",
        "",
        "1. 프로젝트 폴더에서 `02_Run_Admin_Console.bat`를 실행합니다.",
        ...
        "## 2. 반드시 챙길 백업 파일",
        "",
        "- 대시보드 실행 파일: `02_Run_Admin_Console.bat`",
        ...
        "## 5. 발표 전 최종 체크",
        "",
        "- `02_Run_Admin_Console.bat` 실행 확인",
