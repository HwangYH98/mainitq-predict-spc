...
class RiskMonitoringPage(QWidget):
    ...
    def render(self) -> None:
        ...
        if df.empty:
            ...
            self.status_label.setText(f"최근 {min(len(df), 200)}개 관측치를 기준으로 위험 추세를 표시합니다.")
        ...
        set_display_table(
            self.table,
            df.tail(50),
            ["time_step", "simulated_timestamp", "xgboost_probability", "selected_threshold", "risk_status"],
            max_rows=50,
        )
